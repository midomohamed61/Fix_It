...
const WorkerCard = ({
  ...
  const renderStars = (rating: number) => {
    ...
    return (
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        ...
        <span style={{ marginLeft: '0.5rem', color: '#EFB036' }}>{rating}</span>
      </div>
    );
  };
  ...
  const gradientBorderStyle = {
    ...
    backgroundImage: isCardHovered
      ? `linear-gradient(${gradientPosition}deg, #23486A, #3B6790, #4C7B8B, #EFB036, #23486A, #3B6790, #4C7B8B, #EFB036)`
      : 'none',
      ...
      >
        {isCardHovered && (
          <div
            style={{
              ...
              bottom: 0,
              background:
                'linear-gradient(120deg, rgba(255,255,255,0) 0%, rgba(255,255,255,0.1) 50%, rgba(255,255,255,0) 100%)',
              opacity: 0.3,
              zIndex: 1,
              pointerEvents: 'none',
            }}
        ...
        >
          {isLoading ? (
            ...
            <h2 style={{ fontSize: '1.25rem', fontWeight: 'bold', color: '#23486A' }}>{name}</h2>
          )}

          {isLoading ? (
            <ShimmerSkeleton width="80%" height="1rem" borderRadius="0.25rem" />
          ) : (
            <p style={{ fontSize: '0.875rem', color: '#3B6790' }}>{title}</p>
          )}
